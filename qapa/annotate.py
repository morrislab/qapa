...
def update_3prime(feature, min_distance=24, min_intermediate_pas=4, custom=False):
    ...
    if (
        dist_from_three_prime > min_distance
        and not custom
        and int(feature[site_numsamples]) < min_intermediate_pas
    ):
...
def preprocess_gencode_polya(gencode_polya_file):
    ...
    gencode = (
        pybedtools.BedTool(gencode_polya_file)
...
def preprocess_polyasite(polyasite_file, min_polyasite):
    ...
    polyasite = pybedtools.BedTool(polyasite_file).filter(lambda x: x).saveas()
    ...
    if not is_v2:
        ...
        field_index = 9
        polyasite = polyasite.each(_add_chr).each(_move_num_exp).saveas()

    polyasite_te = (
        polyasite.filter(lambda x: int(x[4]) >= min_polyasite)
...
def validate(bedobj, filename):
    ...
    try:
        ...
        raise err(
            f"Error reading the BED file {filename}. Is the file properly formatted?"
        )
...
def main(args, input_filename, fout=sys.stdout):
    ...
    if args.no_annotation:
        overlap_utrs = utrs.each(restore_feature).saveas()
        logger.info("Skipping annotation step")
    else:
        logger.info("Finding intersect")
        overlap_utrs = (
            utrs.intersect(sites, s=True, wa=True, wb=True)
    ...
    if len(overlap_utrs) == 0:
        raise RuntimeError(
            "Failed to find any overlap between UTRs and "
            "annotation. Please check your input files."
        )

    overlap_utrs = (
        sort_bed(overlap_utrs)
        .groupby(
            g=[1, 2, 3, 6, 7, 8, 9], c=[4, 5, 10, 11], o=["collapse"] * 4, delim="|"
        )
