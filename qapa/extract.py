...
class Row:
    def __init__(self, row):
        ...
        if len(items) <= 15:
            raise ValueError(
                "Insufficient number of columns in gene"
        ...
        try:
            ...
            self.chrom = items[2]
            self.strand = items[3]
            self.txStart = int(items[4])
            self.txEnd = int(items[5])
            self.cdsStart = int(items[6])
            self.cdsEnd = int(items[7])
            self.exonStarts = [int(x) for x in [_f for _f in items[9].split(",") if _f]]
            self.exonEnds = [int(x) for x in [_f for _f in items[10].split(",") if _f]]
            self.name2 = items[12]
        except Exception:
            raise ValueError(
                "Unable to parse the input genePred file."
    ...
    def extract_last_exon(self, n=1, min_utr_length=0):
        ...
        if not self.has_intron_in_3utr and self.get_3utr_length() >= min_utr_length:
            if self.strand == "+":
        ...
        if not self.has_intron_in_3utr and self.get_3utr_length() >= min_utr_length:
            if self.strand == "+":
                bed = (
                    [self.chrom]
                    + self.utr3
                    + [name, self.get_3utr_length(), self.strand]
                )
            else:
                bed = (
                    [self.chrom]
    ...
    def get_block_starts(self, n):
        if self.strand == "+":
            return ",".join(
                [str(x - self.exonStarts[-n]) for x in self.exonStarts[-n:]]
            )
...
def main(args, fout=sys.stdout):
    ...
    for row in fileinput.input(
        args.annotation_file[0], openhook=fileinput.hook_compressed
    ):
        n = n + 1

        if fileinput.isfirstline():
            if row.startswith("#bin") or row.startswith("bin"):
                logger.debug("Header detected in genePred file. Assuming UCSC format.")
                continue
            else:
        ...
        if not args.no_skip_random_chromosomes and rowobj.is_on_random_chromosome():
        ...
        if rowobj.chromosome_contains_underscore():
            ...
            if rowobj.chrom not in bad_chroms:
                logger.warning(
                    "Skipping chromosome %s because it contains"
        ...
        try:
            ...
            if not (
                result["Gene type"] == "protein_coding"
        ...
        if bed is not None:
            ...
            logger.debug("Skipping %s (could not extract last exon", rowobj.name)
            c = c + 1
    ...
    if (skipped := c / n) > 0.99:
        raise RuntimeError(
            "Too many entries, %d/%d (%0.2f%%), were skipped. "
            "Are you using the correct database?" % (c, n, 100 * skipped)
        )
    elif skipped > 0.75:
        logger.warning(
            "%d/%d (%0.2f%%) were skipped. Are you using the "
