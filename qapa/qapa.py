...
(.gz) format."""
    ...
    common.add_argument(
        ...
        "--temp",
        type=str,
        help="Set temp directory [{}]".format(tempfile.gettempdir()),
    )
    ...
    common_build.add_argument(
        "--debug",
        action="store_true",
        help="Increase verbosity of log messages for troubleshooting.",
    )
...
(to STDOUT).
    ...
    required.add_argument(
        "--db", type=str, required=True, help="Ensembl gene identifier table"
    )
    required_polya = build_parser.add_argument_group(
        "required named arguments when using public poly(A) site annotations"
    )
    required_polya.add_argument(
        "-g", "--gencode_polya", dest="gencode_polya", help="GENCODE poly(A) site track"
    )
    required_polya.add_argument(
        "-p", "--polyasite", dest="polyasite", help="PolyAsite database"
    )
    ...
    optional.add_argument(
        "-i",
        "--intermediate_polyasite",
        dest="intermediate_polyasite",
        type=int,
        default=4,
    ...
    fasta_parser = subparsers.add_parser(
        "fasta",
        description=desc,
        help="extract FASTA sequences from BED file",
        parents=[common, common_build],
    )
    ...
    optional.add_argument(
        ...
        default=False,
        action="store_true",
        help="Generate a decoy-aware output 3'UTRome FASTA for compatibility with Salmon's >= v1.0.0 selective alignment procedure",
    )
    optional.add_argument(
        "-d",
        "--decoys-output-file",
        type=str,
        dest="decoys_output_file",
        default="decoys.txt",
        help="Name of output decoys file (default: decoys.txt). Ignored if --decoys is not passed",
    )
    ...
    quant_parser = subparsers.add_parser(
        "quant",
        description=desc,
        help="compute PAU for one or more samples",
        parents=[common],
    )
    ...
    optional.add_argument(
        ...
        type=str,
        metavar="FILE",
        help="Save intermediate file of merged samples as FILE",
    )
    ...
    if args.subcommand == "build":
        if (
            args.other is None
            and (args.gencode_polya is None or args.polyasite is None)
            and not args.no_annotation
        ):
            parser.error("Missing arguments: -g and -p flags are mutually inclusive")

        if args.other and (args.gencode_polya or args.polyasite):
            logger.info(
                "Option -o (custom BED) will be used for build phase and "
                "-g and -p will be ignored"
            )

        if not (args.species is None or re.match(r"^[a-zA-Z0-9]+$", args.species)):
...
def build(args):
    ...
    tf1 = tempfile.NamedTemporaryFile(mode="w", prefix="qapa_extract_", delete=False)
    tf2 = tempfile.NamedTemporaryFile(mode="w", prefix="qapa_anno_", delete=False)
    tf3 = tempfile.NamedTemporaryFile(mode="w", prefix="qapa_extend_", delete=False)
...
def quant(args):
    ...
    if not args.save:
        merged_tmp = tempfile.NamedTemporaryFile(prefix="qapa_merge_", delete=False)
