...
class AnnotateTestCase(unittest.TestCase):
    ...
    def test_extend_feature(self):
        length = 1
        feature = self.bed[0]
        target = anno.extend_feature(feature, length=length)
        self.assertEqual(target.start, 74026591 - length)
    ...
    def test_gene_at_beginning_of_chr_1(self):
        ...
        target = anno.restore_feature(target, length=length)
        self.assertEqual(target.start, 0)
    ...
    def test_preprocess_polyasite_v1(self):
        with self.assertLogs("qapa", level="INFO") as cm:
            result = anno.preprocess_polyasite(
                DIR / "files/polyasite_v1.example.bed", 2
            )
    ...
    def test_preprocess_polyasite_v2(self):
        with self.assertLogs("qapa", level="INFO") as cm:
            result = anno.preprocess_polyasite(
                DIR / "files/polyasite_v2.example.bed", 2
            )
